...
export default function DAOTab({ chain }) {
  ...
  const dao = getContract('MobilisDAO');
  ...
  const fetchProposals = useCallback(async () => {
    ...
    try {
      ...
      for (let i = 1; i <= Number(count); i++) {
        ...
        const voted = await dao.hasVoted(i, selectedAccount.address);
        items.push({
          id: Number(p.id),
          title: p.title,
  ...
  const handleCreate = async (e) => {
    ...
    setMsg(null);
    try {
      const tx = await dao.createProposal(form.title, form.description);
      await tx.wait();
          ...
          <div className="card">
            ...
            {proposals.length === 0 ? (
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
                {proposals.map(p => {
                  const isActive = now < p.deadline && !p.executed;
                  const totalVotes = p.votesFor + p.votesAgainst;
                  const forPct = totalVotes > 0 ? (p.votesFor / totalVotes) * 100 : 50;
                  const secondsLeft = Math.max(0, Math.floor((p.deadline - now) / 1000));
                  return (
